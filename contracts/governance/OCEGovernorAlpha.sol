...
// Modified to work in the OCE system

// all votes work on underlying _oceBalances[address], not balanceOf(address)
...
//
// Changes made by OCE after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since OCE supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match OCE as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "OCE Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(oce.initSupply(), 4), 100); } // 4% of OCE

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(oce.initSupply(), 100); } // 1% of OCE
    ...
    constructor(address timelock_, address oce_) public {
        timelock = TimelockInterface(timelock_);
        oce      = OCEInterface(oce_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(oce.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || oce.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = oce.getPriorVotes(voter, proposal.startBlock);
